...
if __name__ == "__main__":
  ...
  if (4 != len(sys.argv)):
    logging.fatal("Usage:  %s model.cfg model.weights classes.data", sys.argv[0])
    sys.exit(1)
  
  config = sys.argv[1]
  weights = sys.argv[2]
  metadata = sys.argv[3]
  logging.debug("Model config file:   %s", config)
  logging.debug("Model weights file:  %s", weights)
  logging.debug("Class metadata file: %s", metadata)
  ...
  #
  # Usage example:
  #   curl http://localhost:5252/detect?kind=jpg&url=http%3A%2F%2Frestcam
  #
